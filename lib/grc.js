...
async function fetchGRCCDC(postal_code) {
  ...
  redis
    ...
    .sadd(cdc, grc)
    .sadd(grc, div)
    .sadd("valid-grc", postal_code)
    .sadd("valid-cdc", postal_code)
    .sadd("valid-div", postal_code)
    .exec();
...
const msfRegex = new RegExp(/var strPos = '(([0-9]{6},)+)';/);
...
async function fetchSSO(postal_code) {
  ...
  var [msfPage, onemapResult] = await Promise.all([
    fetch(`https://www.msf.gov.sg/dfcs/sso/default.aspx?loc=${postal_code}`),
    fetch(
      `https://developers.onemap.sg/msfservices/getSocialService?postal=${postal_code}&dist=10&token=${token}`
    )
  ]);
  var ssoPC = await msfPage.text().then(page => {
    var [, fullstring] = page.match(msfRegex);
    var [relevantPC] = fullstring.split(",");
    return relevantPC;
  });
  var results = await onemapResult
    .json()
  ...
  if (results == undefined || results.length < 2) {
    ...
    results.shift();
    // use the postal code returned
    console.log(`trying to match to ${ssoPC}`);
    var closestSSO = results.find(sso => {
      return sso["ADDRESSPOSTALCODE"] == ssoPC;
    });
    console.log(closestSSO);

    if (closestSSO == undefined) {
      // if it can't be found, then filter by geographical distance
      // Then find the closest SSO in the list
      var closestSSO = results.reduceRight(
        (shortest, current) =>
  ...
  redis.sadd("ssos", sso);
  redis.sadd("valid-sso", postal_code);
  return sso;
}
...
function dataFnFor(type) {
  return async pc => {
    ...
    var [[, valid], [, response]] = await redis
      .multi()
      .sismember(`valid-${type}`, pc)
      .hget(pc, type)
      .exec();
    if (!valid || response == null || response.trim() == "") {
